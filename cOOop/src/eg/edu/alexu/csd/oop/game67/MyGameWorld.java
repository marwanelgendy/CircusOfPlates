...
public class MyGameWorld implements World, Observable {
    ...
    public MyGameWorld(int screenWidth, int screenHeight,Level level, String p) {
        fwo = flyweightGObject.getInstance();
        File f = new File("shapes");
        fwo.setPath(f.list());
        Vector<String> s = new Vector<>();
        for (int i = 0; i < fwo.getPath().length; i++) {
            if ((level.getLevelNum() == 1 && (fwo.getPath()[i].equals("bomb.png") || fwo.getPath()[i].equals("orange.png"))) || (level.getLevelNum() == 2 && fwo.getPath()[i].equals("bomb.png"))) {
                continue;
            } else {
